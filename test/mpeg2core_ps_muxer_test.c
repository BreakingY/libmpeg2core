...
int ps_muxer_h26x_test(const char *file, int stream_type_h26x){
    FILE *fp = fopen(file, "r");
    if(fp == NULL){
        printf("file not exist\n");
        exit(0);
    }
    FILE *ps_fd = fopen("test_out_h26x.ps", "wb");
    if(ps_fd == NULL){
        printf("open test_out_h26x.ps failed\n");
        exit(0);
    }
    ...
    int ret = mpeg2_ps_add_stream(context, stream_type_h26x, NULL, 0);
    if(ret < 0){
    ...
    while (1) {
        ...
        if(mpeg2_ps_packet_muxer(context, frame, frame_size, stream_type_h26x, time_base_convert(pts, 90000), time_base_convert(dts, 90000)) < 0){
...
int ps_muxer_aac_test(const char *file){
    FILE *fp = fopen(file, "r");
    if(fp == NULL){
        printf("file not exist\n");
        exit(0);
    }
    FILE *ps_fd = fopen("test_out_aac.ps", "wb");
    if(ps_fd == NULL){
        printf("open test_out_aac.ps failed\n");
        exit(0);
    }
    // FILE *aac_fd = fopen("ps_muxer_aac_test.aac", "wb");
    mpeg2_ps_context *context = create_ps_context();
    if(!context){
        printf("create_ps_context error\n");
        exit(0);
    ...
    mpeg2_ps_set_write_callback(context, media_write_callback, 1, ps_fd);
    int ret = mpeg2_ps_add_stream(context, STREAM_TYPE_AUDIO_AAC, NULL, 0);
    if(ret < 0){
        printf("mpeg2_ps_add_stream error\n");
        exit(0);
    }
    ...
    while (1) {
        ...
        if(mpeg2_ps_packet_muxer(context, frame, header.aac_frame_length, STREAM_TYPE_AUDIO_AAC, pts, dts) < 0){
            printf("mpeg2_ps_packet_muxer error\n");
        }
...
int main(int argc, char **argv){
    ...
    switch (type){
        case 0:
            ps_muxer_h26x_test(argv[1], STREAM_TYPE_VIDEO_H264);
            break;
        case 1:
            ps_muxer_h26x_test(argv[1], STREAM_TYPE_VIDEO_H265);
            break;
        case 2:
            ps_muxer_aac_test(argv[1]);
            break;
        default:
            break;
    }
